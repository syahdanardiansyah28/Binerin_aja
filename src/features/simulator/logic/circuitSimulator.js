...
const incomingValuesFor = (node, edges, outputMap) => {
  ...
  incoming.forEach((edge) => {
    ...
    const value = outputValue(sourceOutputs, edge.sourceHandle || 'out');
    if (value !== undefined && value !== null) values[edge.targetHandle || 'in'] = value;
  });
  ...
  for (let pass = 0; pass < nodes.length + 2; pass += 1) {
    nodes.forEach((node) => {
      ...
      if (node.type === 'gateNode') {
        ...
        outputMap.set(node.id, { out: value });
        details.set(node.id, { value, outputs: { out: value }, inputValues: values, status: 'Ready', errorMessage: '' });
      }
      ...
      if (node.type === 'halfAdderNode') {
        ...
        const outputs = { sum: values.a ^ values.b, carry: values.a & values.b };
        outputMap.set(node.id, outputs);
        details.set(node.id, { value: outputs.sum, outputs, inputValues: values, status: 'Ready', errorMessage: '' });
      }
      ...
      if (node.type === 'fullAdderNode') {
        ...
        outputMap.set(node.id, outputs);
        details.set(node.id, { value: sum, outputs, inputValues: values, status: 'Ready', errorMessage: '' });
      }
      ...
      if (node.type === 'outputNode') {
        ...
        details.set(node.id, { value: values.in, outputs: { in: values.in }, inputValues: values, status: 'Ready', errorMessage: '' });
      }
    });
  }
