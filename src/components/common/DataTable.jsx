export default function DataTable({ headers, rows, emptyText = 'Belum ada data.' }) {
  return (
    <div className="overflow-x-auto rounded-md border border-linear-border/70">
      <table className="w-full min-w-[360px] border-collapse text-left font-mono text-sm">
        <thead className="bg-linear-surface2 text-linear-text">
          <tr>
            {headers.map((header) => (
        ...
        <tbody className="text-linear-muted">
          {rows.length ? (
            rows.map((row, rowIndex) => (
              <tr key={row.join('-') || rowIndex} className="border-t border-linear-border/70">
            ...
            <tr className="border-t border-linear-border/70">
