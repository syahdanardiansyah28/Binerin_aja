      ...
      <section className="relative overflow-hidden border-b border-white/[0.06]">
        <Container className="grid gap-10 py-12 md:py-16 lg:grid-cols-[minmax(0,0.95fr)_minmax(420px,1fr)] lg:items-center">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-5xl font-medium leading-none text-white md:text-6xl">
              Binerin
            </h1>
          ...
          <div className="grid gap-8 lg:grid-cols-[0.7fr_1fr] lg:items-start">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {simulatorTabs.slice(0, 7).map((tab) => (
                <button
                  key={tab.id}
                  className="min-h-14 rounded-lg border border-white/[0.06] bg-linear-surface px-4 text-left text-sm text-linear-muted transition hover:bg-white/[0.03] hover:text-linear-text"
                  type="button"
...
function CircuitPreview() {
  ...
  return (
    <div className="relative min-h-[360px] overflow-hidden rounded-lg border border-white/[0.06] bg-linear-surface p-5">
      ...
      <div className="relative grid h-full grid-cols-1 gap-4 sm:grid-cols-5 sm:items-center">
        {nodes.map(([label, value], index) => (
          <div key={label} className="relative rounded-lg border border-white/10 bg-linear-bg p-4">
            <p className="font-mono text-xs text-linear-muted">{label}</p>
            <p className="mt-3 break-all font-mono text-2xl text-white">{value}</p>
